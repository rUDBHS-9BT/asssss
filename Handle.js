...
`
                            ...
                            case 'instagram':
                                ...
                                try {
                                    ...
                                    case 's':
										if (isMedia && !message.message.videoMessage || isQuotedImage) {
											const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(message).replace('quotedM','m')).message.extendedTextMessage.contextInfo : message
											await client.downloadAndSaveMediaMessage(encmedia).then(media => {
											ran = getRandom('.webp')
											   sharp(media).resize({
      									      width: 512,
         									   height: 512,
				...
				case 'stikergif':
				case 'sgif':
				 if (isMedia && mek.message.videoMessage.seconds < 11 || isQuotedVideo && mek.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) {
