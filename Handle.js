...
const filterBadword = require('./lib/badword')
const {
    fetchJson
} = require('./lib/fetcher')
const {
...
`
                            ...
                            case 'instagram':
                                ...
                                try {
                                    ...
                                    case 's':
										if (isMedia && !message.message.videoMessage || isQuotedImage) {
											const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(message).replace('quotedM','m')).message.extendedTextMessage.contextInfo : message
											await client.downloadMediaMessage(encmedia).then(media => {
											ran = getRandom('.webp')
											   sharp(media).resize({
      									      width: 512,
         									   height: 512,
        									    ...
        									    fs.writeFileSync('./temp/' + ran, buffer)
												  spawn('webpmux', ['-set', 'exif', './temp/data.exif', './temp/' + ran, '-o', './temp/' + ran ])
 												   .on('exit', () => {
 													client.sendMessage(from, fs.readFileSync('./temp/' + ran), sticker, { quoted: message })
 						 							fs.unlink(media)
     												 fs.unlink('./temp/' + ran)
													    })
         									   })
				...
				case 'sgif':
				 if (isMedia && message.message.videoMessage.seconds < 11 || isQuotedVideo && message.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) {
					...
					const media = await client.downloadMediaMessage(encmedia)
					ran = getRandom('.webp')
     										...
     										canvas.Canvas.trigger(media)
      										 .then(async(buffer) => {
    										    canvas.write(buffer, ran)
													const stream = bufferToStream(ran)
  													ffmpeg(stream)
													  .inputFormat('mp4')
													  .addOutputOptions("-vcodec", "libwebp", "-vf", "scale='min(150,iw)':min'(150,ih)':force_original_aspect_ratio=decrease, format=rgba, fps=15, pad=150:150:-1:-1:color=#00000000", '-lossless', '1', "-loop", "1", "-preset", "default", "-an", "-vsync", "0", "-s", "150:150")
  													.save(`./temp/${ranw}`)
					  								.on('end', () => {
  													  stream.destroy()
 													   spawn('webpmux', ['-set', 'exif', './temp/data.exif', './temp/' + ranw, '-o', './temp/' + ranw])
  													  .on('exit', () => {
   													client.sendMessage(from, (fs.readFileSync('./temp/' + ranw)), sticker, { quoted : message })
  	 											    fs.unlink('./temp/' + ran)
  					 								fs.unlink('./temp/' + ranw)
													   fs.unlink(media)
 												   })
...
`
                                        ...
                                        break
                                        case 'brainly':
                                        	if(args.length < 1) return reply(`Send ${prefix}brainly <text>`)
