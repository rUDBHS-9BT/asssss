...
`
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'F-BOT'; if (!author) author = 'IG : @forfun.197';	
                    ...
                    break
         			...
         			break
                        ...
                        case 'leave':
                            ...
                            break
                        case 'level':
                	        if(!isLeveling) retyrn reply('Leveling Not Enabled')
                            ...
                            case 'instagram':
                                ...
                                try {
                                    ...
                                    if(args.length >= 2){
                                        ...
                                        await addMetadata(author,packname)
                                        reply('done')
                                    	}else{
										...
										if (isMedia && !message.message.videoMessage || isQuotedImage) {
											...
											ran = getRandom('.webp')
												await ffmpeg(`./${media}`)
													...
													.on('end', function () {
														console.log('Finish')
														exec(`webpmux -set exif ${addMetadata('F-BOT', 'IG : @forfun.197')} ${ran} -o ${ran}`, async (error) => {
															if (error) return reply(mess.error.stick)
															client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: message})
															fs.unlinkSync(media)	
															fs.unlinkSync(ran)	
														})
													})
                                    ...
                                    case 'stikergif':
                                    case 'sgif':
                      	              if (isMedia && message.message.videoMessage.seconds < 11 || isQuotedVideo && message.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) {
											...
											const media = await client.downloadAndSaveMediaMessage(encmedia)
												...
												await ffmpeg(`./${media}`)
													...
													.on('error', function (err) {
														...
														reply(`❌ Failed, an error occurred while converting the ${tipe} to a sticker ❌`)
													})
													.on('end', function () {
														console.log('Finish')
														exec(`webpmux -set exif ${addMetadata('F-BOT', 'IG : @forfun.197')} ${ran} -o ${ran}`, async (error) => {
									...
									fs.unlinkSync(media)
										let buffer = Buffer.from(res.base64img, 'base64')
											...
											exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
												...
												exec(`webpmux -set exif ${addMetadata('F-BOT', 'IG : @forfun.197')} ${ranw} -o ${ranw}`, async (error) => {
													if (error) return reply(mess.error.stick)
													client.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: message})
													fs.unlinkSync(ranw)
												})
											})
									})
...
pngttp = './temp/ttp.png'
ran = getRandom('.webp')
fetch(`https://api.areltiyan.site/sticker_maker?text=${body.slice(5)}`, { method: 'GET'})
	.then(async res => {
		result = res.json
		base64Img.img(result.base64, 'temp', 'ttp', function(err, filepath) {
			if (err) return console.log(err)
				exec(`ffmpeg -i ${pngttp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ran}`, (err) => {
					fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
						exec(`webpmux -set exif ${addMetadata('F-BOT', 'IG : @forfun.197')} ${ran} -o ${ran}`, async (error) => {
						if (error) return reply(mess.error.stick)
       ...
       .then(async(buffer) => {
        ...
        await ffmpeg(`./${ran}`)
			...
			.on('error', function (err) {
				...
				fs.unlinkSync(media)
				fs.unlinkSync(ran)
				reply(`❌ Failed, an error occurred while converting the gif to a sticker ❌`)
			})
			.on('end', function () {
				console.log('Finish')
				exec(`webpmux -set exif ${addMetadata('F-BOT', 'IG : @forfun.197')} ${ranw} -o ${ranw}`, async (error) => {
